...
- the router container image to **public ECR**
...
- `PUBLIC_ECR_REPO`: repository name (e.g. `lambda-parallel-router/router`).
...
2. Builds and pushes the router image to public ECR.
3. Renders `bootstrap/template.yaml` with `DefaultRouterImageIdentifier` set to that image URI.
...
- The SAR version is pinned to the router image tag, so each SAR release maps to an exact image.
