# Simple Multiplexer Gateway

A long-running HTTP gateway that **micro-batches** requests per route for a few milliseconds and
...
- **Streaming (NDJSON)**: Lambda is invoked with `InvokeWithResponseStream` and emits NDJSON records.
  The gateway dispatches each record as soon as it arrives (record-level streaming, not interleaved
  chunk streaming).
...
- **Mode B (adapter, recommended)**: wrap an existing handler with a one-line adapter.
  - Node adapter lives in [`lambda-kit/adapter-node/`](lambda-kit/adapter-node/) (package name: `smug-lambda-adapter`).
  - Rust adapter lives in [`lambda-kit/adapter-rust/`](lambda-kit/adapter-rust/) (crate name: `smug-lambda-adapter`, import as `smug_lambda_adapter`).
...
### Gateway config manifest (YAML/JSON)

The gateway reads a **single config manifest** that embeds both gateway settings and the OpenAPI-ish
spec. Keys follow these conventions:

- `GatewayConfig` keys: **PascalCase** (CloudFormation-friendly)
- `x-smug` keys: **camelCase**

Example ([`examples/local/gateway.yaml`](examples/local/gateway.yaml)):
...
- `x-smug.dynamicWait` enables sigmoid-based dynamic batching (see spec for parameters).

## Repository layout

- [`gateway/`](gateway/): Rust gateway (axum) with per-route microbatching.
...
1) Create a gateway config manifest.
   - [`examples/local/gateway.yaml`](examples/local/gateway.yaml) is a starting point.
